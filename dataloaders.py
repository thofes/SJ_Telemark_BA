...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        for i, x in enumerate(files): # Pfade durchgehen und bei einstelligen eine 0 hinzufügen
            if x[-6] == '/':
                files[i] = x[:-5] + '0' + x[-5:]
        files = sorted(files) # sortieren mit zweistelligen Zahlen
        

        for i, x in enumerate(files): # bei eigentlich einstelligen die 0 wieder entfernen
            if x[-6] == '0':
                files[i] = x[:-6] + x[-5:]
